				...
				menuName: {
					...
					isUnique: {
						...
						queryFieldList:function(value, element){
							return [{fieldName:"menuName",operator:"=", fieldValue:"'" + value +"'"}];
						}
					}
				},
				location: {
    ...
    tableOption.table.on("click",".editRow",function() {
        ...
        var jcUpdate = updateData(tableOption,item,
    		...
    		function(updateForm,item,content){
	    		...
	    		content.find("input[name='menuName']").rules("remove");
	    		content.find("input[name='menuName']").rules("add",{
					required: true,
					isUnique: {
						tableName:"tb_system_menu", 
						queryFieldList:function(value, element){
							return [{fieldName:"menuName",operator:"=", fieldValue:"'" + value +"'"},
								{fieldName:"menuId",operator:"!=", fieldValue:"'" + item.menuId +"'"}];
							}
						}
					}
	    		);
