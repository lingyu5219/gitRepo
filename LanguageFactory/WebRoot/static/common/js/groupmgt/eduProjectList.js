$(function() {
	var tableOption = {
		//ajax查询数据的http请求url
		queryUrl : basePath + "groupmgt/queryEduProjectList",
		//ajax删除数据的http请求EduProjectType
		delUrl : basePath + "groupmgt/delEduProject",	
		//增加页面
		addPageUrl : basePath + 'forward?page=groupmgt/eduProjectAdd',
		//ajax增加数据的http请求url
		addUrl : basePath + "groupmgt/addEduProject",
		//修改页面
		updatePageUrl : basePath + 'forward?page=groupmgt/eduProjectModify',
		//ajax修改数据的http请求url
		updateUrl : basePath + "groupmgt/updateEduProject",
		//查看页面
		detailPageUrl : basePath + 'forward?page=groupmgt/eduProjectDetail',
		//Table对象
		table : $('#dataListTable'),
		//查询表单Form对象
		form : $("#queryForm"),
	...
	initBasicSelect2({
		...
		callback: function(data){
        	...
        	$("select.staffGartenIdSelect2").select2({data:result});
        }
	});
