$(function() {
	var tableOption = {
		//ajax查询数据的http请求url
		queryUrl : basePath + "groupmgt/queryClassTypeList",
		//ajax删除数据的http请求ClassType
		delUrl : basePath + "groupmgt/delClassType",	
		//增加页面
		addPageUrl : basePath + 'forward?page=groupmgt/classTypeAdd',
		//ajax增加数据的http请求url
		addUrl : basePath + "groupmgt/addClassType",
		//修改页面
		updatePageUrl : basePath + 'forward?page=groupmgt/classTypeModify',
		//ajax修改数据的http请求url
		updateUrl : basePath + "groupmgt/updateClassType",
		//查看页面
		detailPageUrl : basePath + 'forward?page=groupmgt/classTypeDetail',
		//Table对象
		table : $('#dataListTable'),
		//查询表单Form对象
		form : $("#queryForm"),
		...
		columns : [//对应thead里面的序列
	        ...
	        {"data":'classTypeId',visible:false},
	        {"data":'classTypeName'},
	        {"data":'description'},
  ...
  //增加
    ...
    tableOption.table.on("click",".delRow",function() {
        ...
        var params = {classTypeId:item.classTypeId};
        //调用删除方法，向服务器发送请求
