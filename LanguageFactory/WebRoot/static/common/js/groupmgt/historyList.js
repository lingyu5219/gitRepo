$(function() {
	var tableOption = {
		//ajax查询数据的http请求url
		queryUrl : basePath + "groupmgt/queryHistoryList",
		//ajax删除数据的http请求history
		delUrl : basePath + "groupmgt/delHistory",	
		//增加页面
		addPageUrl : basePath + 'forward?page=groupmgt/historyAdd',
		//ajax增加数据的http请求url
		addUrl : basePath + "groupmgt/addHistory",
		//修改页面
		updatePageUrl : basePath + 'forward?page=groupmgt/historyModify',
		//ajax修改数据的http请求url
		updateUrl : basePath + "groupmgt/updateHistory",
		//查看页面
		detailPageUrl : basePath + 'forward?page=groupmgt/historyDetail',
		//Table对象
		table : $('#dataListTable'),
		//查询表单Form对象
		form : $("#queryForm"),
		...
		columns : [//对应thead里面的序列
	        ...
	        {"data":'historyTitle'},
	        {"data":'historyContent',className:'hidden-xs hidden-sm',
	        	"render":function(data,type,full,callback) {
	        		return '<span title="'+data +'">'+ (data.length > 30 ? data.substr(0,30) + "......" : data) +'</span>';;
	        	}
	        },
	        {"data":'historyDate',className:'hidden-xs hidden-sm',"render":function(data,type,full,callback) {
        		return full.historyDate = data.substr(0,10);
            }},
    ...
    tableOption.table.on("click",".delRow",function() {
        ...
        var params = {historyId:item.historyId};
        //调用删除方法，向服务器发送请求
