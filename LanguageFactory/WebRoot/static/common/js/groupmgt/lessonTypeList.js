$(function() {
	var tableOption = {
		//ajax查询数据的http请求url
		queryUrl : basePath + "groupmgt/queryLessonTypeList",
		//ajax删除数据的http请求LessonType
		delUrl : basePath + "groupmgt/delLessonType",	
		//增加页面
		addPageUrl : basePath + 'forward?page=groupmgt/lessonTypeAdd',
		//ajax增加数据的http请求url
		addUrl : basePath + "groupmgt/addLessonType",
		//修改页面
		updatePageUrl : basePath + 'forward?page=groupmgt/lessonTypeModify',
		//ajax修改数据的http请求url
		updateUrl : basePath + "groupmgt/updateLessonType",
		//查看页面
		detailPageUrl : basePath + 'forward?page=groupmgt/lessonTypeDetail',
		//Table对象
		table : $('#dataListTable'),
		//查询表单Form对象
		form : $("#queryForm"),
		...
		columns : [//对应thead里面的序列
	        ...
	        {"data":'lessonTypeId',visible:false},
	        {"data":'lessonTypeName'},
  ...
  //增加
    ...
    tableOption.table.on("click",".delRow",function() {
        ...
        var params = {lessonTypeId:item.lessonTypeId};
        //调用删除方法，向服务器发送请求
    ...
    tableOption.table.on("click",".detailRow",function() {
    	//要查看哪一行数据对象
        var item = dataTable.row($(this).closest('tr')).data();
        item.titleSuffix = "课程类型" + item.lessonTypeId;
