$(function () {
	...
	var tableOption = {
		...
		//增加页面
		addPageUrl : basePath + 'forward?page=groupmgt/messageAdd',
		//ajax增加数据的http请求url
		addUrl : basePath + "groupmgt/addMessage",
		//修改页面
		updatePageUrl : basePath + 'forward?page=groupmgt/messageModify',
		//ajax修改数据的http请求url
		updateUrl : basePath + "groupmgt/updateMessage",
		//查看页面
		detailPageUrl : basePath + 'forward?page=groupmgt/messageDetail',
		//视频选择页面
		mediaSearchPageUrl : basePath + 'forward?page=groupmgt/mediaSearch',
		//Table对象
		table : $('#dataListTable'),
		//查询表单Form对象
		form : $("#queryForm"),
	    	...
	    	function(content){
        		...
        		initBasicSelect2({
        			...
        			callback: function(data){
                    	...
                    	var option = $("<option selected=\"selected\" value=\"-1\">请选择所属校园</option>");
	    				content.find("select.msgGartenIdSelect2").prepend(option);
                    }
        		});
	    		...
	    		content.find("input[name='isGroupMsg']").bootstrapSwitch({
	    			...
	    			onSwitchChange:function(event,state){  
	    	            if(state==true){  
	    	            	content.find("select.msgGartenIdSelect2").val(-1).trigger("change");
	    	            	//content.find("select.msgGartenIdSelect2").prop("disabled", true);
	    	            	content.find("select.msgGartenIdSelect2").next().hide();
	    	            	...
	    	            	content.find("select.msgGartenIdSelect2").next().show();
    ...
    tableOption.table.on("click",".detailRow",function() {
        ...
        var jcDetail = detailData(tableOption,item,function(detailForm,item,content){
    		...
    		if(item.msgGartenId <= 0){
    			...
    			content.find("input[name='msgGartenName']").hide();
    			content.find("input[name='isGroupMsg']").bootstrapSwitch('disabled', true);
    		} else {
    			content.find("input[name='msgGartenName']").parent().prev().prev().text("所属校园");
    			content.find("input[name='msgGartenName']").parent().prev().hide();
    			content.find("input[name='msgGartenName']").parent().removeClass("col-md-6").addClass("col-md-9");
    		}
